...
static void *fault_worker(void *argument)
{
	...
	for (int round = 0; round < FAULT_ROUNDS; round++) {
		void *view = mmap(NULL, worker->view_size, PROT_READ, MAP_PRIVATE,
				  worker->peephole_fd, 0);
		if (view == MAP_FAILED) {
...
static int test_self_multithread_fault(void)
{
	...
	for (int i = 0; i < thread_count; i++)
		ASSERT(pthread_create(&threads[i], NULL, fault_worker, &worker) == 0,
		       "failed to spawn fault worker");
...
static int test_race_unmap_vs_fault(void)
{
	...
	ASSERT(pthread_create(&reader_thread, NULL, race_reader, &reader) == 0,
	       "failed to spawn race reader");
	...
	ASSERT(MIRILLA_COMMAND_IS_OK(mirilla_engage(mirilla_fd, getpid(), &target_id)),
	       "self-engage failed after the race");
	ASSERT(MIRILLA_COMMAND_IS_OK(mirilla_disengage(mirilla_fd, target_id)),
	       "self-disengage failed after the race");
	close(mirilla_fd);
	return 0;
}
...
static int test_target_exit_while_reading(void)
{
	...
	if (child_pid == 0) {
		...
		_exit(target_child_process(pipe_to_child[PIPE_READ], pipe_from_child[PIPE_WRITE]) ==
				      0 ?
			      EXIT_SUCCESS :
			      EXIT_FAILURE);
	}
	...
	ipc_message_t msg;
	ASSERT(recv_message(pipe_from_child[PIPE_READ], &msg) == 0 && msg.type == MSG_READY,
	       "target child never became ready");
	...
	mirilla_map_target_id_t target_id = 0;
	ASSERT(MIRILLA_COMMAND_IS_OK(mirilla_engage(mirilla_fd, child_pid, &target_id)),
	       "failed to engage the target child");
	...
	ASSERT(verify_region_faultable(view, target_size, MAGIC_VALUE_3) == 0,
	       "view does not mirror the target region");
	...
	ASSERT(send_message(pipe_to_child[PIPE_WRITE], MSG_EXIT, 0, 0) == 0,
	       "failed to command the target exit");

	int child_status;
	ASSERT(waitpid(child_pid, &child_status, 0) == child_pid, "failed to reap the target");
	ASSERT(WIFEXITED(child_status) && WEXITSTATUS(child_status) == EXIT_SUCCESS,
	       "target child exited abnormally");
