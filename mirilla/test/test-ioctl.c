...
static int test_bad_magic(void)
{
	...
	errno = 0;
	int rejection_status = expect_ioctl_rejection(ioctl(mirilla_fd, cmd, NULL),
						      MIRILLA_ENOTSUPP, "foreign-magic ioctl");
...
static int test_bad_category(void)
{
	...
	errno = 0;
	int rejection_status = expect_ioctl_rejection(ioctl(mirilla_fd, cmd, NULL),
						      MIRILLA_ENOTSUPP, "foreign-category ioctl");
...
static int test_bad_map_command(void)
{
	...
	errno = 0;
	int rejection_status = expect_ioctl_rejection(ioctl(mirilla_fd, cmd, NULL), ENOTTY,
						      "unknown map command");
...
static int test_bad_argument_pointer(void)
{
	...
	errno = 0;
	int rejection_status = expect_ioctl_rejection(ioctl(mirilla_fd, cmd, NULL), EFAULT,
						      "engage with a NULL argument");
...
static int test_engage_nonexistent_pid(void)
{
	...
	int rejection_status =
		expect_ioctl_rejection(mirilla_engage(mirilla_fd, NONEXISTENT_PID, &target_id),
				       ESRCH, "engage of a nonexistent pid");
...
static int test_peephole_malformed_ranges(void)
{
	...
	for (size_t i = 0; i < sizeof(malformed_ranges) / sizeof(malformed_ranges[0]); i++) {
		...
		if (expect_ioctl_rejection(mirilla_peephole(mirilla_fd, target_id,
							    ...
							    malformed_ranges[i].end_address, NULL,
							    NULL),
...
static int test_double_disengage(void)
{
	...
	ASSERT(MIRILLA_COMMAND_IS_OK(mirilla_disengage(mirilla_fd, target_id)),
	       "first disengage failed");

	errno = 0;
	int rejection_status = expect_ioctl_rejection(mirilla_disengage(mirilla_fd, target_id),
						      ENOENT, "double disengage");
