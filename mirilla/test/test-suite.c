...
static int child_process(int pipe_in, int pipe_out)
{
    ...
    if (send_message(pipe_out, MSG_READY, (unsigned long)regions[0].addr, TEST_REGION_SIZE) < 0) {
    ...
    while (1) {
        ...
        case MSG_ALLOCATE_REGION: {
            ...
            send_message(pipe_out, MSG_ACK, (unsigned long)regions[num_regions].addr, size);
            num_regions++;
            break;
        }
        ...
        case MSG_WRITE_DATA: {
            ...
            if (region_idx < 0 || region_idx >= num_regions || !regions[region_idx].addr) {
            ...
            for (size_t i = 0; i < regions[region_idx].size / sizeof(unsigned int); i++) {
                data[i] = value + i;
            }

            /* Force memory to be paged in */
            for (size_t i = 0; i < regions[region_idx].size; i += 4096) {
                volatile unsigned char *p = (volatile unsigned char *)regions[region_idx].addr + i;
                (void)*p;
            }
        ...
        case MSG_REMAP_REGION: {
            ...
            void *new_addr =
                allocate_test_region(regions[region_idx].size, regions[region_idx].current_value);
            if (!new_addr) {
        ...
        case MSG_VERIFY: {
            ...
            int result =
                verify_region(regions[region_idx].addr, regions[region_idx].size, expected);
...
#define MAX_PEEPHOLES 8
    ...
    TEST_START("MAP_ENGAGE");
    {
        ...
        if (!MIRILLA_COMMAND_IS_OK(ret)) {
            char error_msg[256];
            snprintf(error_msg, sizeof(error_msg), "ioctl failed with ret=%ld, errno=%d (%s)", ret,
    ...
    TEST_START("Read Initial Mirror Data");
    {
        ...
        if (verify_region(peepholes[0].mapped_addr, peepholes[0].size, MAGIC_VALUE_1) == 0) {
    ...
    TEST_START("Multiple Peepholes - Same Target");
    {
        ...
        if (!MIRILLA_COMMAND_IS_OK(ret)) {
            char error_msg[256];
            snprintf(error_msg, sizeof(error_msg), "failed to create second peephole: ret=%ld",
                     ret);
            ...
            peepholes[1].mapped_addr =
                mmap(NULL, peepholes[1].size, PROT_READ, MAP_PRIVATE, peepholes[1].fd, 0);
            ...
            if (peepholes[1].mapped_addr == MAP_FAILED) {
                ...
                printf("[PARENT] Created peephole[1]: id=%lu, fd=%d, mapped at "
                       "%p\n",
                       (unsigned long)peepholes[1].id, peepholes[1].fd, peepholes[1].mapped_addr);

                /* Verify both peepholes show same data */
                if (verify_region(peepholes[0].mapped_addr, TEST_REGION_SMALL, MAGIC_VALUE_1) ==
                        0 &&
                    verify_region(peepholes[1].mapped_addr, TEST_REGION_SMALL, MAGIC_VALUE_1) ==
                        0) {
    ...
    TEST_START("Child Write Propagation - Multiple Peepholes");
    {
        ...
        if (recv_message(pipe_in, &msg) < 0 || msg.type != MSG_ACK) {
            ...
            int peephole0_ok =
                verify_region(peepholes[0].mapped_addr, peepholes[0].size, MAGIC_VALUE_2) == 0;
            int peephole1_ok =
                verify_region(peepholes[1].mapped_addr, peepholes[1].size, MAGIC_VALUE_2) == 0;
    ...
    if (child_region2_addr) {
        TEST_START("Multiple Peepholes - Different Regions");
        {
            ...
            union mirilla_map_peephole_io peephole_io = { 0 };
            peephole_io.argument.target_id = target_id;
            peephole_io.argument.start_address = (uint64_t)(unsigned long)child_region2_addr;
            peephole_io.argument.end_address =
            ...
            if (!MIRILLA_COMMAND_IS_OK(ret)) {
                char error_msg[256];
                snprintf(error_msg, sizeof(error_msg), "failed to create peephole: ret=%ld", ret);
                ...
                peepholes[2].mapped_addr =
                    mmap(NULL, peepholes[2].size, PROT_READ, MAP_PRIVATE, peepholes[2].fd, 0);
                ...
                if (peepholes[2].mapped_addr == MAP_FAILED) {
                    ...
                    if (verify_region(peepholes[0].mapped_addr, TEST_REGION_SMALL, MAGIC_VALUE_2) ==
                            0 &&
                        verify_region(peepholes[2].mapped_addr, child_region2_size,
                                      MAGIC_VALUE_3) == 0) {
                        TEST_PASS("Multiple Peepholes - Different Regions");
                    } else {
                        TEST_FAIL("Multiple Peepholes - Different Regions", "data mismatch");
                    }
                }
            }
        }
    ...
    TEST_START("Multiple Modifications Across Regions");
    {
        ...
        if (verify_region(peepholes[0].mapped_addr, TEST_REGION_SMALL, MAGIC_VALUE_4) != 0) {
            all_passed = 0;
        }
        ...
        if (num_peepholes >= 3 && peepholes[2].mapped_addr) {
            ...
            if (verify_region(peepholes[2].mapped_addr, child_region2_size, MAGIC_VALUE_5) != 0) {
                all_passed = 0;
            }
        }
    ...
    TEST_START("Access After Remap - Fault Expected");
    {
        ...
        if (faultable_probe_u32(peepholes[0].mapped_addr, &probe_value) == CATALEJO_OUTCOME_ERROR) {
    ...
    TEST_START("Mirror Read Performance");
    {
        ...
        void *perf_mapped = mmap(NULL, TEST_REGION_SIZE, PROT_READ, MAP_PRIVATE, perf_fd, 0);
...
int main(int argc __attribute__((unused)), char *argv[] __attribute__((unused)))
{
    ...
    if (child_pid == 0) {
        ...
        status = child_process(pipe_parent_to_child[PIPE_READ], pipe_child_to_parent[PIPE_WRITE]);
